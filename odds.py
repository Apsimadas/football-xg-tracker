...
# "active" (in season) today — off-season leagues are skipped automatically,
# which keeps request usage low without needing manual seasonal updates.
SOCCER_ALLOW = {
...
def select_target_keys(active_keys):
    ...
    targets = []  # (key, display_name, sport_group)
    for key, title, group in active_keys:
        if group == "Soccer" and key in SOCCER_ALLOW:
            targets.append((key, SOCCER_ALLOW[key], "Ποδόσφαιρο"))
        elif group == "Basketball" and key in BASKETBALL_ALLOW:
            targets.append((key, BASKETBALL_ALLOW[key], "Μπάσκετ"))
        elif group == "Tennis" and (key.startswith("tennis_atp_") or key.startswith("tennis_wta_")):
            targets.append((key, title, "Τένις"))
    return targets
...
def build_odds_matches_for_date(target_date, xg_matches, api_key):
    ...
    targets = select_target_keys(active_keys)
    results = []
